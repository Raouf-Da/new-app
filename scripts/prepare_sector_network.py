...
def define_spatial(nodes, options):
    ...
    # Cast to string index to prevent TypeError when concatenating with string labels
    import pandas as pd
...
if __name__ == "__main__":
    ...
    # Load population layout (needed for spatial definitions)
    ...
    # Set carrier of AC loads (which we removed, but this is good practice)
    existing_nodes = [node for node in nodes if node in n.loads.index]
    if len(existing_nodes) < len(nodes):
        print(f"Warning: For {len(nodes) - len(existing_nodes)} of {len(nodes)} nodes there were no load nodes found in network and were skipped.")
    ...
    # --- Add Base Components We Need ---
    add_co2(n, costs)
    remove_elec_base_techs(n)
    add_generation(n, costs, 0, None, None) # Adds conventional stubs (which will be 0)
    # remove_carrier_related_components(n, carriers_to_drop=["H2", "battery"])
    add_hydrogen(n, costs)
    add_storage(n, costs)

    # --- ADD OUR CUSTOM GREEN STEEL H2 LOAD ---
    logger.info("Adding custom load for green steel (H2 only)")
    
    # Our calculated demand in average MW (for Hydrogen Mode)
    steel_h2_demand_mw = 890 
    # Yearly electricity demand in MWh (for Electricity Mode)
    ac_demand_mwh_yr = 1000000
    simulation_mode = "Hydrogen"
    
    # Desalination overrides
    desalination_efficiency_m3_mwh = 285.0
    desalination_capex_eur_m3h = 150000
    
    # Coordinates for Tosyali Algeria (Bethioua Port Mineralier)
    tosyali_coords = [-0.2039293, 35.8003028] # [longitude, latitude]
    
    # Coordinates for Bellara / El Milia
    bellara_coords = [6.24442, 36.753961] # [longitude, latitude]
    
    custom_coords = [-2.3181152343750004, 31.68143311662596]
    
    # ACTIVE SITES
    active_sites = ["Bellara"]
    
    all_sites = {
        "Tosyali": tosyali_coords,
        "Bellara": bellara_coords,
        "Custom Location": custom_coords
    }
    sites = {k: v for k, v in all_sites.items() if k in active_sites}

    closest_buses = {}
    
    for site_name, coords in sites.items():
        # Find the closest AC bus
        bus_distances = ((n.buses[n.buses.carrier=='AC'].x - coords[0])**2 + (n.buses[n.buses.carrier=='AC'].y - coords[1])**2)**0.5
        closest_bus_ac = bus_distances.idxmin()
        closest_buses[site_name] = closest_bus_ac
        
        if simulation_mode == "Electricity":
            ac_mw = ac_demand_mwh_yr / 8760.0
            logger.info(f"Attaching {ac_demand_mwh_yr} MWh/yr AC load (Avg {ac_mw:.1f} MW) to {closest_bus_ac} ({site_name})")
            n.madd(
                "Load",
                [f"{closest_bus_ac} AC Plant {site_name}"],
                bus=closest_bus_ac,
                carrier="AC",
                p_set=ac_mw
            )
        else:
            logger.info(f"Attaching {steel_h2_demand_mw} MW H2 load to closest AC bus for {site_name}: {closest_bus_ac}")
            # Add the H2 load
            n.madd(
                "Load",
                [f"{closest_bus_ac} H2 Steel Plant {site_name}"],
                bus=f"{closest_bus_ac} H2", # Connects to the H2 bus at that node
                carrier="H2 for industry",
                p_set=steel_h2_demand_mw # This is a static, 24/7 load
            )
    # --- END OF CUSTOM LOAD ---
# ---> START OF WATER & DESALINATION LOGIC <---
    
    if simulation_mode == "Hydrogen":
        # 1. Create the Water Carrier globally
        n.add("Carrier", "water")
        
        for site_name, closest_bus_ac in closest_buses.items():
            logger.info(f"Adding water infrastructure and RO desalination for {site_name} to: {closest_bus_ac}")
            
            # Create Water Bus for the specific node
            if f"{closest_bus_ac} Water" not in n.buses.index:
                n.add("Bus", f"{closest_bus_ac} Water", carrier="water")

        # 2. Model the Desalination Plant (Electricity -> Water)
        n.add(
            "Link", 
            f"{closest_bus_ac} Desalination Plant {site_name}",
            bus0=closest_bus_ac,                 # Draws electricity from the local AC grid
            bus1=f"{closest_bus_ac} Water",      # Outputs water
            carrier="desalination",
            efficiency=desalination_efficiency_m3_mwh,  # m3 of water per MWh of electricity
            p_nom_extendable=True,
            capital_cost=desalination_capex_eur_m3h     # Estimated Algerian CAPEX (EUR per m3/h)
        )

        # 3. Add a Water Tank for storage arbitrage
        n.add(
            "Store", 
            f"{closest_bus_ac} Water Tank {site_name}",
            bus=f"{closest_bus_ac} Water",
            carrier="water storage",
            e_nom_extendable=True,
            capital_cost=50                      # Estimated CAPEX (EUR per m3)
        )

        # 4. Add the dedicated Plant Electrolyzer consuming BOTH electricity and water
        n.add(
            "Link", 
            f"{closest_bus_ac} Dedicated Electrolyzer {site_name}",
            bus0=closest_bus_ac,
            bus1=f"{closest_bus_ac} H2",
            bus2=f"{closest_bus_ac} Water",
            carrier="electrolysis-dedicated",
            efficiency=0.82,                     # Primary Output: H2 energy
            efficiency2=-0.221,                  # Secondary Input: Water consumed (m3 per MWh)
            p_nom_extendable=True,
            capital_cost=600000                  # Base CAPEX for electrolyzer
        )
    # ---> END OF WATER & DESALINATION LOGIC <---
    # --- All other sectors are now skipped ---
    # add_heat(n, costs)
    # add_biomass(n, costs)
